...
{ \
    ...
    switch (pcm_type) {
        case pcm_unsigned_byte_type: {
            unsigned char const *bl = l, *br = r;
            int i;
            for (i = 0; i < nsamples; i++) {
                // https://stackoverflow.com/questions/24449957/converting-a-8-bit-pcm-to-16-bit-pcm
                sample_t const xl = (*bl - 0x80) << 8;
                sample_t const xr = (*br - 0x80) << 8;
...
int
lame_encode_buffer_byte(
        lame_global_flags *gfp,
        unsigned char pcm_l[], unsigned char pcm_r[], const int nsamples,
        unsigned char *mp3buf, const int mp3buf_size
) {
    //FLOAT const norm = 128.0;
    return lame_encode_buffer_template(gfp, pcm_l, pcm_r, nsamples, mp3buf, mp3buf_size, pcm_unsigned_byte_type, 1, 1.0);
}

int
lame_encode_buffer_interleaved_byte(
        lame_global_flags *gfp,
        unsigned char pcm[], int nsamples,
        unsigned char *mp3buf, int mp3buf_size
) {
    return lame_encode_buffer_template(gfp, pcm, pcm + 1, nsamples, mp3buf, mp3buf_size, pcm_unsigned_byte_type, 2, 1.0);
}
